...
CREATE OR REPLACE FUNCTION set_alpha(alpha float) RETURNS void AS $$
BEGIN
EXECUTE format('CREATE OR REPLACE FUNCTION get_alpha() RETURNS float AS '' SELECT %s::float '' LANGUAGE sql IMMUTABLE', alpha);
END
$$
LANGUAGE plpgsql;
