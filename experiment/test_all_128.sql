...
CREATE TABLE test_smlar_4 (id INT primary key, hmval BIT(128), hmarr TEXT[]);

INSERT INTO test_smlar_4 (id, hmval, hmarr)
SELECT id, gen_bit(val::int[])::bit(128), gen_arr(gen_bit(val::int[]),16)
from test_hsp;

CREATE INDEX idx_hmarr_test_4 ON test_smlar_4 USING GIN(hmarr _text_sml_ops );

CREATE TABLE test_smlar_8 (id INT primary key, hmval BIT(128), hmarr TEXT[]);

INSERT INTO test_smlar_8 (id, hmval, hmarr)
SELECT id, gen_bit(val::int[])::bit(128), gen_arr(gen_bit(val::int[]),8)
from test_hsp;
...
BEGIN
    ...
    EXECUTE format('SELECT * from test_hsp where hamming_distance(val, $1) < $2 ')using '{53,16,48,0,0,0,0,0,0,0,0,224,9,19,5,33}'::hmcode, r ;
    ...
    FOR I IN 1..num LOOP
        ...
        IF r<8 THEN
            ...
            WHERE hmarr %% $1
            AND LENGTH(REPLACE(BITXOR($2::bit(128), hmval)::TEXT, $4, $5)) <= $3') using smlar_4_arr,smlar_4_query,r,'0','';
            end_time = clock_timestamp();
            smlar_4_time = smlar_4_time + age(end_time,start_time);
        END IF;

        IF r<16 THEN
            EXECUTE format('set smlar.threshold = %s;', 1.0-1.0/16*r);
            ...
            WHERE hmarr %% $1
            AND LENGTH(REPLACE(BITXOR($2::bit(128), hmval)::TEXT, $4, $5)) <= $3') using smlar_8_arr,smlar_8_query,r,'0','';
            end_time = clock_timestamp();
...
CREATE TABLE test_vector (id int primary key, val vector(128));

INSERT INTO test_vector SELECT id, ('['||add_delim(gen_bit(val::int[]))||']')::vector(128)
from test_hsp; 
...
BEGIN
    ...
    FOR I IN 1..array_upper(probes, 1) LOOP
        ...
        FOR J IN 1..array_upper(query_arr, 1) LOOP
            ...
            start_time = clock_timestamp();
            EXECUTE format('SELECT id, val <-> $1::vector(128) dis
            FROM test_vector ORDER BY dis LIMIT 10') using query_val;
            end_time = clock_timestamp();
            vector_time = vector_time + age(end_time,start_time);

            EXECUTE format('CREATE TEMP TABLE test AS select id, pow(dis, 2)::int dis from (SELECT id, val <-> $1::vector(128) dis
...
BEGIN
    ...
    FOR I IN 0..7 LOOP
        ...
        FOR J IN 1..array_upper(query_arr, 1) LOOP
            ...
            start_time = clock_timestamp();
            EXECUTE format('SELECT id, LENGTH(REPLACE(BITXOR($2::bit(128), hmval)::TEXT, $4, $5)) dis  FROM test_smlar_4
            WHERE hmarr %% $1
            order by dis limit $3') using smlar_4_arr,smlar_4_query,k,'0','';
            end_time = clock_timestamp();
            smlar_4_time = smlar_4_time + age(end_time,start_time);

            EXECUTE format('CREATE TEMP TABLE test AS SELECT id, LENGTH(REPLACE(BITXOR($2::bit(128), hmval)::TEXT, $4, $5)) dis  FROM test_smlar_4
            WHERE hmarr %% $1
            order by dis limit $3') using smlar_4_arr,smlar_4_query,k,'0','';
...
BEGIN
    ...
    FOR I IN 0..15 LOOP
        ...
        bf_time = '0 second';
        -- RAISE NOTICE '%', probes[I];
        EXECUTE format('set smlar.threshold = %s;', 1.0-1.0/16*I);
        -- RAISE NOTICE '%', probes[I];
        FOR J IN 1..array_upper(query_arr, 1) LOOP
            ...
            start_time = clock_timestamp();
            EXECUTE format('SELECT id, LENGTH(REPLACE(BITXOR($2::bit(128), hmval)::TEXT, $4, $5)) dis  FROM test_smlar_8
            WHERE hmarr %% $1
            order by dis limit $3') using smlar_8_arr,smlar_8_query,k,'0','';
            end_time = clock_timestamp();
            smlar_8_time = smlar_8_time + age(end_time,start_time);

            EXECUTE format('CREATE TEMP TABLE test AS SELECT id, LENGTH(REPLACE(BITXOR($2::bit(128), hmval)::TEXT, $4, $5)) dis  FROM test_smlar_8
            WHERE hmarr %% $1
            order by dis limit $3') using smlar_8_arr,smlar_8_query,k,'0','';
        ...
        END LOOP;
            ...
            -- RAISE NOTICE 'num %',num;
            -- RAISE NOTICE 'k %',k;
            RAISE NOTICE 'thres: %', 1.0-1.0/16*I;
            recall = recall/num/k;
